...
struct Decide {
    static func main() {
        ...
        guard args.count >= 3 else {
            fail("usage: decide.swift <review.md> <approve|request-changes> [notes]", 2)
        }
        let url = URL(fileURLWithPath: args[1])
        guard let decision = Review.Decision(rawValue: args[2]) else {
            fail("unknown decision: \(args[2])", 2)
        }
        ...
        guard Review.isReview(url) else {
            fail("not a review document: \(url.path)", 1)
        }

        do {
            try Review.decide(decision, notes: notes, for: url)
            print("decided: \(decision.rawValue)")
        } catch {
            fail("failed: \(error)", 1)
        }
    }
